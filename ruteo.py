...
def duracion(
    ...
    for p in orden:
        ...
        tramo = rutas.minutos(desde, p.punto, hora + int(t) // 60 - int(t0) // 60, vehiculo)
        # La disrupcion es FISICA: un cierre estira el tramo de verdad, aqui, en la
        # ruta rapida. No depende de que ningun modelo opine nada.
        t += tramo * activos.factor_tiempo(rutas.ZONA_DE[desde], rutas.ZONA_DE[p.punto])
        if p.tipo == "pickup":
...
def mejor_ruta(
    ...
    if len(paradas) <= 1:
        return paradas, duracion(pos, paradas, hora, t0, vehiculo, activos)
    ...
    if len(resto) > MAX_PERMUTAR:
        ...
        return _por_insercion(pos, fija, resto, hora, t0, vehiculo, activos)
    ...
    for perm in permutations(resto):
        ...
        if not _valida(candidato):
            continue
        t = duracion(pos, candidato, hora, t0, vehiculo, activos)
        if t < mejor_t:
...
def _por_insercion(pos, fija, resto, hora, t0, vehiculo="moto", activos=shocks.NINGUNO):
    orden = [fija]
    for nueva in resto:
        mejor, mejor_t = None, float("inf")
        for i in range(1, len(orden) + 1):
            ...
            if not _valida(tuple(cand)):
                continue
            t = duracion(pos, cand, hora, t0, vehiculo, activos)
            if t < mejor_t:
                mejor, mejor_t = cand, t
        orden = mejor or (orden + [nueva])
    return orden, duracion(pos, orden, hora, t0, vehiculo, activos)
...
def costo_marginal(
    ...
    _, sin = mejor_ruta(pos, ruta, hora, t0, vehiculo, activos)
    con_ruta, con = mejor_ruta(pos, ruta + nuevas, hora, t0, vehiculo, activos)
