  ...
  return (
    <div style={{ padding: '40px', fontFamily: 'sans-serif', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <p style={{ lineHeight: 1.6, color: '#444' }}>
        Habla en Español, Inglés o Hindi. El agente detectará tu idioma, pensará usando Gemini 2.5 Flash Lite y responderá con su propia voz.
      </p>

      <div style={{ marginTop: '30px', padding: '20px', border: '1px solid #ddd', borderRadius: '8px', background: '#f9f9f9', textAlign: 'center' }}>
        ...
        >
          {processing ? '🧠 Procesando...' : (recording ? '🛑 Detener y Enviar' : '🎤 Presionar para Hablar')}
        </button>

        {errorMsg && (
          <div style={{ marginTop: '20px', color: '#dc2626', background: '#fee2e2', padding: '10px', borderRadius: '4px' }}>
            {errorMsg}
          </div>
        )}

        <div style={{ marginTop: '30px', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: '#eff6ff', padding: '15px', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
            <strong>Tú dijiste:</strong>
            <p style={{ margin: '10px 0 0', fontSize: '16px', color: '#1e3a8a' }}>{transcription || '...'}</p>
          </div>

          <div style={{ background: '#f0fdf4', padding: '15px', borderRadius: '8px', border: '1px solid #bbf7d0' }}>
            <strong>Nuez respondió:</strong>
            <p style={{ margin: '10px 0 0', fontSize: '16px', color: '#166534' }}>{reply || '...'}</p>
          </div>
        </div>
