...
def get_engine(force_retry: bool = False) -> Optional[Engine]:
    ...
    try:
        ...
        _db_connected = True
        logger.info(f"Conectado exitosamente a TigerData/PostgreSQL en {DATABASE_URL.split('@')[-1]}")
    except Exception as e:
        logger.warning(f"No se pudo conectar a TigerData/PostgreSQL ({e}). Usando almacén en memoria para simulación.")
        _engine = None
        _db_connected = False
    ...
    try:
        with engine.begin() as conn:
            ...
            try:
                ...
                logger.info(f"TimescaleDB no disponible en este servidor ({e}); usando PostgreSQL estándar.")
            ...
            if has_timescale:
                try:
                    conn.execute(text("SELECT create_hypertable('trafico_calles', 'tiempo', if_not_exists => TRUE);"))
                except Exception as e:
        ...
        try:
            with engine.begin() as conn:
                for r in records:
                    conn.execute(
                        text("""
                            INSERT INTO trafico_calles (tiempo, calle_nombre, factor_retraso, delay_segundos, velocidad_kmh, motivo, coords)
                            VALUES (:tiempo, :calle_nombre, :factor_retraso, :delay_segundos, :velocidad_kmh, :motivo, :coords)
                        """),
                        {
                            "tiempo": r["tiempo"],
    ...
    if engine and is_connected():
        try:
            with engine.begin() as conn:
                conn.execute(
                    text("""
                        INSERT INTO incidentes_viales (id, inicio, fin, calle, tipo, factor_penalizacion, motivo, alerta_voz, coords)
                        VALUES (:id, :inicio, :fin, :calle, :tipo, :factor_penalizacion, :motivo, :alerta_voz, :coords)
    ...
    if engine and is_connected():
        try:
            ...
            with engine.connect() as conn:
                result = conn.execute(
                    text("""
                        SELECT calle_nombre, factor_retraso, delay_segundos, velocidad_kmh, motivo, coords
                        FROM trafico_calles
...
def get_active_incidents(hora_str: str) -> List[Dict[str, Any]]:
    ...
    if engine and is_connected():
        try:
            ...
            logger.warning(f"Error consultando incidentes en TigerData: {e}. Usando fallback de memoria.")

    # Fallback de memoria
    active = []
    for inc in _MEMORY_INCIDENTS:
        h_inicio = inc["inicio"].strftime("%H:%M") if isinstance(inc["inicio"], datetime) else str(inc["inicio"])[-5:]
        h_fin = inc["fin"].strftime("%H:%M") if isinstance(inc["fin"], datetime) else str(inc["fin"])[-5:]
