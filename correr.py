...
CFG = ConfigTurno(duracion_min=120, ancla=Punto("Tec", *TEC), margen_min=10,
                  vehiculo="moto", seed=1, hora_inicio=14)
...
def un_turno(seed: int):
    ...
    for d in res.decisiones[:14]:
        ...
        print(f"  t={d.t:>3}  {marca} {d.oferta_id}  "
    ...
    print(f"\n  ofertas: {len(res.ofertas)}   entregas: {res.entregas}   "
          f"rechazos: {res.rechazos}")
    print(f"  ganado: ${res.ganado:.0f}   ocupado: {res.minutos_ocupado}/{cfg.duracion_min} min"
...
def muchos(n: int):
    ...
    print(f"  ganancia media   : ${statistics.mean(ganancias):.0f}"
