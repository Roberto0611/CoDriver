...
and contains no expected answers.
    ...
    # check an order stream CSV before feeding it to your simulator
...
# order_offered's required fields, minus 'event': every row in an order stream
# CSV is an offered order, so the constant column carries no information.
CSV_REQUIRED = ["order_id", "sim_time", "zone_pickup", "zone_dropoff",
                "distance_pickup_km", "distance_delivery_km", "base_pay_mxn",
                "surge_multiplier", "vehicle"]
CSV_NUMERIC = ["zone_pickup", "zone_dropoff", "distance_pickup_km",
               "distance_delivery_km", "base_pay_mxn", "est_tip_mxn",
               "surge_multiplier", "restaurant_prep_min", "weight_kg",
               "volume_liters", "estimated_pickup_min", "estimated_delivery_min"]


def check_order_stream_csv(path: str) -> tuple[list[str], int]:
    """Validate an order stream CSV. Each row is one order_offered."""
    errs: list[str] = []
    with Path(path).open(newline="", encoding="utf-8") as fh:
        ...
        for key in CSV_REQUIRED:
            if key not in cols:
                errs.append(f"missing required column {key!r}")

        seen: set[str] = set()
        n = 0
        for n, row in enumerate(reader, 1):
            for key in CSV_REQUIRED:
                if key in cols and not str(row.get(key, "")).strip():
                    errs.append(f"row {n}: required field {key!r} is empty")

            oid = str(row.get("order_id", "")).strip()
            if oid and oid in seen:
                errs.append(f"row {n}: duplicate order_id {oid!r}")
            seen.add(oid)

            if row.get("vehicle") and row["vehicle"] not in VEHICLES:
                errs.append(f"row {n}: vehicle must be one of {sorted(VEHICLES)}, "
                            f"got {row['vehicle']!r}")
            if row.get("platform") and row["platform"] not in PLATFORMS:
                errs.append(f"row {n}: platform must be one of {sorted(PLATFORMS)}, "
                            f"got {row['platform']!r}")

            # Empty means absent for optional fields; only non-empty must parse.
            for key in CSV_NUMERIC:
                ...
                if val:
                    try:
                        ...
                        errs.append(f"row {n}: {key!r} must be numeric, got {val!r}")
    return errs, n
...
def main() -> int:
    ...
    ap.add_argument("--order-stream-csv", dest="order_stream_csv")
    args = ap.parse_args()

    if not any([args.event_log, args.responses, args.endpoint, args.order_stream_csv]):
    ...
    if args.order_stream_csv:
        e, n = check_order_stream_csv(args.order_stream_csv)
        errs += e
        print(f"  order stream csv: {args.order_stream_csv}")
        print(f"  orders: {n}")
